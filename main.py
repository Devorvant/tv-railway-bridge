...
def bybit_set_isolated_margin_mode():
    ...
    try:
        ...
        # Sometimes it returns error if already set; we log and continue
...
def bybit_get_position_size(symbol: str) -> float:
    ...
    # For linear, positions can return one record; size and side show direction
    p = pos_list[0]
...
def calc_qty(symbol: str) -> float:
    ...
    if equity <= 0 or price <= 0:
        raise HTTPException(status_code=400, detail=f"cannot fetch equity/price (equity={equity}, price={price})")
...
def place_market(symbol: str, side: str, qty: float, reduce_only: bool):
    ensure_bybit_ready()
    # NOTE: qty precision/step is instrument-specific. For first tests it's OK,
    # but later we'll round by lotSizeFilter.
...
def close_full_position(symbol: str):
    ...
    qty = abs(pos)
    if pos > 0:
        # long -> close by Sell reduceOnly
        return place_market(symbol, "Sell", qty, reduce_only=True)
    else:
        # short -> close by Buy reduceOnly
...
async def handle_webhook(req: Request):
    ...
    try:
        if action == "LONG":
            # Optional: close opposite position first (if short)
            pos = bybit_get_position_size(symbol)
            if pos < 0:
            ...
            last_result = {"action": "LONG", "bybit": result}
        ...
        if action == "SHORT":
            ...
            last_result = {"action": "SHORT", "bybit": result}
            return {"ok": True, "bybit": result}

        # Close signals (под твой Pine можно расширять)
        if action in ("CLOSE", "CLOSE_LONG", "STOP_LONG", "STOP_SHORT", "CLOSE_SHORT"):
            result = close_full_position(symbol)
            last_result = {"action": action, "bybit": result}
            return {"ok": True, "bybit": result}
